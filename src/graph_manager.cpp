...
//    poses_ralative_to <<0.570647,	0.074308,	0.817820,	0.054933,              //best match
//                          0.091349,	0.983966,	-0.153145,	0.167663,
//                         -0.816093,	0.162100,	0.554713,	0.057849,
//                          0.000000,	0.000000,	0.000000,	1.000000;
//  poses_ralative_to <<0.553333,  0.0474556,   0.831608,  0.0951716,
//                      0.0855219,  0.989863,  -0.113391,   0.204548,
//                     -0.828559,   0.133863,   0.543665, -0.0588255,
//                            0,          0,          0,          1;
...
//   poses_ralative_to << 0.473786,  0.100923,  0.874838,  0.138913,        //loop2
//                       -0.0189064,  0.994348, -0.104471,  0.299062,
//                       -0.880437, 0.0329567,  0.473016, 0.0135915,
//                            0,         0,         0,         1;
}
...
QList<int> GraphManager::getPotentialEdgeTargetsWithDijkstra(const Node* new_node, int sequential_targets, int geodesic_targets,
                                                             int sampled_targets, int predecessor_id, bool include_predecessor)
{
...
void GraphManager::resetGraph(){
    ...
    sequential_edges = 0;
    found_loopclosure = 0;
    cam2_has_ID_in_graph.clear ();
    cam2_poses_ralative_to_cam1.clear ();
}
/*NEW
                                   ...
                                   bool& edge_to_keyframe)///Output:contains the best-yet of the pairwise motion estimates for the current node
{
    ...
    if(ps->get<double>("min_translation_meter") > 0.0 ||   //not go into this loop
    ...
    if(found_loopclosure ==1 && new_node->cam_id ==1) seq_cand = localization_only_ ? 0 : ps->get<int>("predecessor_candidates") ;
    int geod_cand = ps->get<int>("neighbor_candidates");
    int samp_cand = ps->get<int>("min_sampled_candidates");

    //lligen added  calibrate part
    if(ps->get<bool>("calibrate_kinect") == 1 )
    {
        if(new_node->cam_id == 1){
            if(predecessor_matched){  //0
        ...
        }else{//new_node->cam_id == 2
            ROS_INFO("cam_id is %d, ~~~~%d",new_node->cam_id,new_node->id_);
            vertices_to_comp = getPotentialEdgeTargetsWithDijkstra(new_node, seq_cand-1, geod_cand+1, samp_cand+2, sequentially_previous_id, true);//false

            for (int id_of_id = (int) vertices_to_comp.size() - 1; id_of_id >= 0; id_of_id--) {
                if(cam2_has_ID_in_graph.contains (vertices_to_comp.at (id_of_id)))
                    vertices_to_comp.removeAt (id_of_id);
            }
        }
    }
     //lligen added  switchcam part
    if(ps->get<bool>("few_feature_slam") == 1 )
    {
        ROS_INFO("get vertex to compare ,cam_id is %d, ~~~~%d",new_node->cam_id,new_node->id_);
        vertices_to_comp = getPotentialEdgeTargetsWithDijkstra(new_node, seq_cand, geod_cand, samp_cand, sequentially_previous_id, true);
        ROS_INFO("get vertex to compare ,cam_id is %d, ~~~~%d",new_node->cam_id,new_node->id_);
    }
    ...
    if (ps->get<bool>("concurrent_edge_construction")) 
    {
        ...
        for (int i = 0; i < results.size(); i++)
        {
            ...
            if (mr.edge.id1 >= 0 ) {
              ...
              if (isSmallTrafo(mr.edge.transform, delta_time.toSec()) &&//1
...
bool GraphManager::addNode(Node* new_node) 
{
  ...
  bool found_match;
  //lligen revised
  if(firstnode_afs && new_node->cam_id==2){// switch to cam2
      firstnode_afs = 0;
      new_node->id_ = graph_.size();
      new_node->seq_id_ = next_seq_id++; // allways incremented, even if node is not added
      earliest_loop_closure_node_ = new_node->id_;
      ...
      //construct edge
      MatchingResult mr;
      Node* pre_node = graph_[graph_.size()-1];
      mr.edge.id1 = pre_node->id_;
      mr.edge.id2 = new_node->id_;
      mr.edge.transform = poses_ralative_to.cast<double>();
      mr.edge.informationMatrix = Eigen::Matrix<double,6,6>::Identity() * 1;//e-9;
      addEdgeToG2O(mr.edge,pre_node,new_node, true,true, motion_estimate);
      found_match =1;
      ROS_INFO("switched~~~ to ~~~~cam2 node %d, %d", new_node->cam_id,new_node->id_);
  }else if (firstnode_afs && new_node->cam_id==1) {// back to cam1
      firstnode_afs = 0;
      new_node->id_ = graph_.size();
      new_node->seq_id_ = next_seq_id++; // allways incremented, even if node is not added
      earliest_loop_closure_node_ = new_node->id_;
      ...
      mr.edge.id2 = new_node->id_;
      mr.edge.transform = poses_ralative_to.cast<double>();
      mr.edge.transform = mr.edge.transform.inverse();
      mr.edge.informationMatrix = Eigen::Matrix<double,6,6>::Identity() * 1;//e-9;
      addEdgeToG2O(mr.edge,pre_node,new_node, true,true, motion_estimate);
      found_match =1;
      ROS_INFO("~~~~back to cam1~~~ node %d, %d", new_node->cam_id,new_node->id_);
  }else{
     found_match = nodeComparisons(new_node, motion_estimate, edge_to_last_keyframe_found);
  }
...
//    new_edges_.append(qMakePair(edge.id1, edge.id2));
    ...
    if(abs(edge.id1 - edge.id2) > ParameterServer::instance()->get<int>("predecessor_candidates") ){  //lligen revised  ParameterServer::instance()->get<int>("predecessor_candidates")
      loop_closures_edges++;
    } else {
      sequential_edges++;
    }
    //lligen added
    if( loop_closures_edges % 5 == 0 ){//cam_id=2 的node加到graph里面，单独保存cam_id=2 的node的位姿估计，变换矩阵
        found_loopclosure = 1;
        Q_EMIT loopclosure_found ();
    }
    if (n2->cam_id ==2 && !cam2_has_ID_in_graph.contains(n2->id_)){
        cam2_has_ID_in_graph.push_back(n2->id_);
        ROS_INFO("~~~~~~~~cam2_has_ID_in_graph~~~~~~~%d",n2->id_);
        }
...
//lligen added
    ...
    ParameterServer* ps = ParameterServer::instance();
    if(ps->get<bool>("calibrate_kinect") == 1 ){
        cam2_poses_ralative_to_cam1.reserve (cam2_has_ID_in_graph.size ());
        for (graph_it it = graph_.begin(); it !=graph_.end(); ++it){
          Node *node = it->second;
          if (node->cam_id == 2){
              g2o::VertexSE3* v1 = dynamic_cast<g2o::VertexSE3*>(optimizer_->vertex( node->vertex_id_));
              g2o::VertexSE3* v2 = dynamic_cast<g2o::VertexSE3*>(optimizer_->vertex( node->getPairnode ()->vertex_id_));
              if(v1 && v2){
                  ROS_INFO("%d <-> %d",node->vertex_id_,node->getPairnode ()->vertex_id_);

                 //Pa = Tras' * Pb   v2->estimate() * Trans = v1->estimate()
                 QMatrix4x4 P1 = eigenTF2QMatrix(v1->estimate());
                 QMatrix4x4 P2 = eigenTF2QMatrix(v2->estimate());
                 QMatrix4x4 ralative_pose = P2.inverted ()* P1;
                 if(! cam2_poses_ralative_to_cam1.contains(ralative_pose)){
                     cam2_poses_ralative_to_cam1.push_back(ralative_pose);
                     printQMatrix4x4("cam2_poses_ralative_to_cam1 is",ralative_pose);

                         tf::Transform previous = eigenTransf2TF(v2->estimate());
                         tf::Transform combined = eigenTransf2TF(v1->estimate());
                         tf::Transform poses_ralative_to_cam1= previous.inverse () * combined ;
                         printTransform("cam2_poses_ralative_to_cam1",poses_ralative_to_cam1);
...
void GraphManager::firstnode_afterswitch()
{
     ROS_INFO("~~NEED switch~~~");
    firstnode_afs = 1;
}
