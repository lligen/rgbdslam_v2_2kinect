...
void ui_connections(QObject* ui, GraphManager* graph_mgr, OpenNIListener* listener)
{
  ...
  //lligen added
  QObject::connect(graph_mgr, SIGNAL(loopclosure_found()), listener, SLOT(loopclosure_found()));
  QObject::connect(listener, SIGNAL(firstnode_afterswitch()), graph_mgr,  SLOT(firstnode_afterswitch()));
}
