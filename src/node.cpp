  ...
  initial_node_matches_(0)
{
...
#endif
  {
    ...
    projectTo3D(feature_locations_2d_, feature_locations_3d_, depth, cam_info);//2d features -> 3d features
    /* for(unsigned int i = 0; i < feature_locations_2d_.size(); i++){
                                       ...
                                       std::vector<cv::DMatch>& matches) const
{
  ...
  const int g2o_iterations = ParameterServer::instance()->get<int>( "g2o_transformation_refinement");   //0
...
void Node::setPairnode(Node *pair_node)
{
    pairnode = pair_node;
}
Node *Node::getPairnode() const
{
    return pairnode;
}
