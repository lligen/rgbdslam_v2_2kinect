...
: graph_mgr_(graph_mgr),
  ...
  found_loopclosure(0),  //lligen added
  working_cam(0),
  cam_needswitch_(0),
  few_feature(0)
{
  ...
  std::string visua_tpc2 = ps->get<std::string>("topic_image_mono2");
  std::string depth_tpc2 = ps->get<std::string>("topic_image_depth2");
  std::string cinfo_tpc2 = ps->get<std::string>("camera_info_topic2");
  ros::NodeHandle nh;
  ...
  if(bagfile_name.empty()){
    ...
    //No cloud, but visual image and depth
            ...
            && !visua_tpc2.empty() && !depth_tpc2.empty() && !cinfo_tpc2.empty())
    {   
        visua_sub_ = new image_sub_type(nh, visua_tpc, q);
        depth_sub_ = new image_sub_type(nh, depth_tpc, q);
        cinfo_sub_ = new cinfo_sub_type(nh, cinfo_tpc, q);
        visua_sub2_ = new image_sub_type(nh, visua_tpc2, q);  //lligen revised
        depth_sub2_ = new image_sub_type(nh, depth_tpc2, q);
        cinfo_sub2_ = new cinfo_sub_type(nh, cinfo_tpc2, q);
        no_cloud_sync_ = new message_filters::Synchronizer<NoCloudSyncPolicy>(NoCloudSyncPolicy(q),  *visua_sub_, *depth_sub_, *cinfo_sub_,
                                                                              *visua_sub2_, *depth_sub2_, *cinfo_sub2_);
        no_cloud_sync_->registerCallback(boost::bind(&OpenNIListener::noCloudCallback, this, _1, _2, _3, _4, _5, _6));
  ...
  //lligen added   --  initialization
  detector_2 = createDetector(ps->get<std::string>("feature_detector_type"));
  extractor_2 = createDescriptorExtractor(ps->get<std::string>("feature_extractor_type"));
}
...
void OpenNIListener::loadBag(std::string filename)
{
  ...
  {   
    ...
    no_cloud_sync_ = new message_filters::Synchronizer<NoCloudSyncPolicy>(NoCloudSyncPolicy(q),  *rgb_img_sub_, *depth_img_sub_, *cam_info_sub_,
                                                                          *rgb_img_sub2_, *depth_img_sub2_, *cam_info_sub2_);
    no_cloud_sync_->registerCallback(boost::bind(&OpenNIListener::noCloudCallback, this, _1, _2, _3, _4, _5, _6));
...
void OpenNIListener::noCloudCallback (const sensor_msgs::ImageConstPtr& visual_img_msg,
                                      ...
                                      const sensor_msgs::ImageConstPtr &visual_img_msg2,
                                      const sensor_msgs::ImageConstPtr &depth_img_msg2,
                                      const sensor_msgs::CameraInfoConstPtr &cam_info_msg2)
{
  ScopedTimer s(__FUNCTION__);
  ROS_WARN_ONCE_NAMED("eval", "First RGBD-Data Received");
  ...
  if(asyncFrameDrop(depth_img_msg->header.stamp, visual_img_msg->header.stamp))  //  true async
    return;
  ...
  Node* node_ptr = new Node(visual_img, depth_float_img, depth_mono8_img_, cam_info_msg, depth_img_msg->header, detector_, extractor_,1);

  //lligen added  switchcam part
  if(ps->get<bool>("few_feature_slam") == 1 ) {
      if (node_ptr->feature_locations_2d_.size() < ps->get<int>("min_matches")) {//20
        //  graph_mgr_->save_A_Clouds("piont_white");
          ROS_INFO("Skipping node ~~~~~~only %zu features (minimum is %d)",node_ptr->feature_locations_2d_.size(), ps->get<int>("min_matches"));
          few_feature = 1;
      }else{ few_feature = 0;}
      if(working_cam ==0){
          if(few_feature == 1){
              switchCam();   //lligen added
          }else{
              noCloudCameraCallback(visual_img, depth_img_msg->header, node_ptr);
          }
      }
  }
  ...
  {
      noCloudCameraCallback(visual_img, depth_img_msg->header, node_ptr);
      ROS_INFO("~~~~~~~~found_loopclosure %d",found_loopclosure);
      cv::Mat depth_float_img2;
      cv::Mat visual_img2;
      if (found_loopclosure){
          found_loopclosure = 0;
          depth_float_img2 = cv_bridge::toCvCopy(depth_img_msg2)->image;
          depthToCV8UC1(depth_float_img2, depth_mono8_img2_);
          //cv::Mat visual_img2;
          if(image_encoding_ == "bayer_grbg8"){
              cv_bridge::toCvShare(visual_img_msg2);
              //ROS_INFO("Converting from Bayer to RGB");
              cv::cvtColor(cv_bridge::toCvCopy(visual_img_msg2)->image, visual_img2, CV_BayerGR2RGB, 3);
          } else{
              ROS_DEBUG_STREAM("Encoding: " << visual_img_msg2->encoding);
              visual_img2 =  cv_bridge::toCvCopy(visual_img_msg2)->image;
          }
          Node* node_ptr2 = new Node(visual_img2, depth_float_img2, depth_mono8_img2_, cam_info_msg2, depth_img_msg2->header, detector_2, extractor_2,2);
          node_ptr2->setPairnode (node_ptr);
          noCloudCameraCallback(visual_img2, depth_img_msg2->header, node_ptr2);
      }
  }
  ...
  if(ps->get<bool>("few_feature_slam") == 1 )
  {
    if (cam_needswitch_){
        cam_needswitch_ = 0;
        ROS_INFO("CAM switched successful~~~~working cam is %d~~",working_cam);
        working_cam = !working_cam;
        if(working_cam ==0){
            ROS_INFO("CAM switched successful~~~~working cam is %d~~",working_cam);
            noCloudCameraCallback(visual_img, node2_copy->header_, node2_copy);//here _1 is of little useage
        }
    }
    cv::Mat depth_float_img2;
    cv::Mat visual_img2;
    ...
    node_ptr2->setPairnode (node_ptr);
    if(working_cam ==1){
        ROS_INFO("process SECOND FRAME~~~~AFTER switch~~%d",node_ptr2->cam_id);
        noCloudCameraCallback(visual_img2, depth_img_msg2->header, node_ptr2);
        if(few_feature == 0){//enough features, back to cam1
            ROS_INFO_ONCE("~~~~enough features, back to cam1~~~~~~");
            switchCam();
        }
    }
    ...
    if (cam_needswitch_){
        ...
        if(working_cam ==0){
            ROS_INFO("CAM switched successful~~~~working cam is %d~~",working_cam);
            noCloudCameraCallback(visual_img, node_ptr->header_, node_ptr);//here _1 is of little useage
            //noCloudCameraCallback(visual_img, depth_img_msg->header, node_ptr);
        }
    }

    //node_copy = node_ptr;
    node2_copy = node_ptr2;
  }
}
...
void OpenNIListener::noCloudCameraCallback(cv::Mat visual_img,std_msgs::Header depth_header,Node* node_ptr)
{
...
void OpenNIListener::loopclosure_found()
{
    found_loopclosure = 1;
}

void OpenNIListener::switchCam()
{
     ROS_INFO("~~~~~~~~NEED TO SWITCH CAM~~~~~~~~~~~~~~");
    cam_needswitch_ = 1;
    Q_EMIT firstnode_afterswitch();
}
