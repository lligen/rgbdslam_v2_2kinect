...
void ParameterServer::defaultConfig() {
  ...
  //lligen added
  addOption("calibrate_kinect",             static_cast<bool> (true),                 "if true,calibrate 2 kinects' ralative pose matrix");
  addOption("few_feature_slam",             static_cast<bool> (false),                 "if true,slam in a few_feature environment");

  // Input data settings
  addOption("topic_image_mono",              std::string("/camera1/rgb/image_color"),    "Color or grayscale image ros topic");
  addOption("camera_info_topic",             std::string("/camera1/rgb/camera_info"),    "Required for backprojection if no pointcloud topic is given");
  addOption("topic_image_depth",             std::string("/camera1/depth/image_raw"),        "Depth image ros topic");
  addOption("topic_image_mono2",              std::string("/camera2/rgb/image_raw"),    "Color or grayscale image ros topic");
  addOption("camera_info_topic2",             std::string("/camera2/rgb/camera_info"),    "Required for backprojection if no pointcloud topic is given");
  addOption("topic_image_depth2",             std::string("/camera2/depth/image_raw"),        "Depth image ros topic");
  ...
  addOption("drop_async_frames",             static_cast<bool> (true),                 "Check timestamps of depth and visual image, reject if not in sync ");
  ...
  addOption("optimize_landmarks",            static_cast<bool> (true),                 "Consider the features as landmarks in optimization. Otherwise optimize camera pose graph only");
